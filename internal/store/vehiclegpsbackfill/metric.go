...
// it periodically counts how many Vehicle rows still hold a plaintext
// GPS column without ciphertext, per column, and exposes the count on
// `/metrics` as `vehicle_gps_plaintext_remaining_total{column=...}`.
//
...
func NewPlaintextGauge(reg prometheus.Registerer, pool *pgxpool.Pool, interval time.Duration, logger *slog.Logger) *PlaintextGauge {
	g := prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name: "vehicle_gps_plaintext_remaining_total",
		Help: "Vehicle rows where <col> is non-NULL but <col>Enc is NULL — i.e. plaintext GPS coordinates not yet encrypted at rest. Drains to 0 once the MYR-63 dual-write rollout backfills every row.",
	}, []string{"column"})
	reg.MustRegister(g)
...
// column). Used by the backfill CLI immediately after a Run() so the
// post-run metric value matches the stdout report.
...
func (p *PlaintextGauge) refreshOnce(ctx context.Context) {
	...
	if err != nil {
		if p.logger != nil {
			p.logger.Warn("vehicle-gps plaintext-remaining query failed",
