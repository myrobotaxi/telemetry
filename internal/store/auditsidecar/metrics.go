...
var failureReasons = []string{"put", "enqueue_full", "closed", "other"}
...
func NewPrometheusMetrics(reg prometheus.Registerer) *PrometheusMetrics {
	...
	failures := prometheus.NewCounterVec(prometheus.CounterOpts{
		...
		Help: "Total AuditLog sidecar write failures by reason. reason=put means the S3 PutObject (against AWS S3 or any S3-compatible backend, e.g. Supabase Storage) failed after all retries. reason=enqueue_full means the internal channel was at capacity. reason=closed means Emit was called after Close (graceful shutdown drop, distinct from queue overflow). reason=other means an unexpected error.",
	}, []string{"reason"})
