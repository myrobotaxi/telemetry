...
- marks the matching sessions cut off **synchronously**, so no further frame is enqueued for them from that instant (the flag is read by `Client.hasVehicle`, the single choke point both `Broadcast` and `BroadcastMasked` funnel through); then
