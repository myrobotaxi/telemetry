...
| `chargeState` | `string` (enum) | On REST `/snapshot` only (transitional, pending DB persistence) | -- | P0 | `charge` | Tesla proto field **2** (`ChargeState`). Enum: `Unknown`, `Disconnected`, `NoPower`, `Starting`, `Charging`, `Complete`, `Stopped`. Live WS wiring completed by MYR-40 on 2026-04-22; REST snapshot remains transitional-null until the cross-repo Prisma DB-persistence follow-up lands -- see `websocket-protocol.md` §10 DV-03. |
