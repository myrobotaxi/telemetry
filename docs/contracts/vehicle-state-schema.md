...
> **Ventilated-seat capability rule ([MYR-299](https://linear.app/myrobotaxi/issue/MYR-299)).** Nothing in the stack carries a "does this car have cooled seats" capability field — `vehicle_config.seat_type` is never decoded, and `seatVentEnabled` is a **runtime on/off**, not a capability. The contracted stand-in is the **presence** of `seatCoolerLeft`/`seatCoolerRight`: a car without ventilated seats never emits Tesla protos **237/238** at all, while a car with them emits a value that **includes `0`** (present-but-off). Consumers therefore derive the capability as `seatCoolerLeft != null || seatCoolerRight != null`, and MUST NOT read the level's *value* as the signal.
>
