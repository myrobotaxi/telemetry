...
The **REST `in_service` bool** is read edge-triggered (NOT polled): the [`ServiceStatusMonitor`](../../internal/telemetry/service_status_monitor.go) subscribes to `ConnectivityEvent` and, on each connect/disconnect edge, fires **one** debounced (per-VIN, ~45s cooldown) authoritative `GET /api/1/vehicles/{vin}` read and persists `status = in_service` to `Vehicle.status` when the flag is true. This leg only ever **writes** `in_service` (never clears to `parked`): a `false` flag leaves the last-known status untouched — the live pipeline (drive-ended→parked, gear frames) owns the transition back out of in-service, and active driving always wins in the live derivation. Read failures are non-fatal (last-known preserved). This is the first code path to emit/persist the `in_service` enum value, which has always been part of the `status` enum (additive-safe — no consumer change).
