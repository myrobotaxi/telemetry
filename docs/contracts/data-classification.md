...
### 1.13 go_vehicle_control_state table (Go-owned, MYR-269; extended MYR-273, MYR-279, MYR-274, MYR-298, MYR-303, MYR-308, MYR-316, MYR-320)
...
| `permission` | `TEXT` (enum, CHECK) | P0 | No | Yes | Cumulative access tier: `live` \| `live_history` \| `rides`. An authorization tier describing a relationship, not identifying data — same classification as the sibling `role` on the wire |
...
### P1 fields with log-redaction only (no app-level encryption, 29 columns)
