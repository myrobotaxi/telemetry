...
| `updated_at` | `TIMESTAMPTZ NOT NULL DEFAULT now()` | P0 | No | Yes | **When this grant's capability flags were last modified by an owner patch** ([MYR-451](https://linear.app/myrobotaxi/issue/MYR-451), migration 0032). Stamped unconditionally by `queryPatchShare`, in the same statement that moves the flags, so the timestamp cannot disagree with the values it dates; the lifecycle transitions keep their own columns and no trigger competes for this one. Added because `allow_rides` was a bare boolean beside a `suspended_at` that recorded WHEN — so a withdrawn ride capability could not be placed in time, and an urgent beta report ("this viewer rode after I turned Rides off") could not be resolved either way from the row. A clock reading about a row, not about a person: it names no party and describes no location, exactly like `created_at` beside it |
...
### P1 fields with log-redaction only (no app-level encryption, 23 columns)
