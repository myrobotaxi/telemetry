...
**Verify before destroy.** The purge does not trust a NULL-check on the ciphertext sibling. For every row it decrypts the ciphertext and compares it against the plaintext it is about to remove, scrubbing only on a match. Rows that are unsealed, mismatched, or undecryptable are counted, logged **without their values**, and left completely intact — the purge never removes the only copy of anything. `-dry-run` performs the identical verification and writes nothing. Idempotent.
