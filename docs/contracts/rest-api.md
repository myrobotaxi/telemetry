...
> **Divergence (DV-20):** A subset of the SDK-surface REST endpoints in §6 / §7 remain unmounted on the Go server. `GET /api/vehicles` (§7.0) landed first (MYR-91, 2026-05-10); `GET /api/vehicles/{vehicleId}/snapshot` (§7.1) and `GET /api/vehicles/{vehicleId}/drives` (§7.2) landed in MYR-133 (2026-06-03). `GET /api/drives/{driveId}` (§7.3) and `GET /api/drives/{driveId}/route` (§7.4) remain pending. See §10.
...
`GET /api/vehicles` (§7.0) is mounted by the Go server as of MYR-91 (2026-05-10). `GET /api/vehicles/{vehicleId}/snapshot` (§7.1) and `GET /api/vehicles/{vehicleId}/drives` (§7.2) are mounted as of MYR-133 (2026-06-03). The remaining Go-server-owned paths (`/drives/{id}`, `/drives/{id}/route`) are PLANNED — see DV-20 in §10. `GET /api/users/me/export` (and the §7.6 / §7.5 endpoints) is served by the Next.js app per §10 DV-23 and is NOT in scope for the Go server's DV-20 mount.
...
| **DV-20** | **Open (reduced)** | SDK-surface REST endpoints not yet mounted on the Go server | Two of the four Go-server-owned endpoints in §7 remain unmounted: `GET /api/drives/{driveId}` (§7.3) and `GET /api/drives/{driveId}/route` (§7.4). `GET /api/vehicles/{vehicleId}/snapshot` (§7.1) and `GET /api/vehicles/{vehicleId}/drives` (§7.2) landed in MYR-133 (2026-06-03). `GET /api/vehicles` (§7.0) landed in MYR-91 (2026-05-10). (The §7.5 invite endpoints and §7.6 `DELETE /api/users/me` are NOT in DV-20's scope -- per DV-23 (RESOLVED 2026-05-08, MYR-69) they are served by the Next.js app and are tracked under MYR-70 / MYR-71 / MYR-73 instead. The Go server returning 404 on those paths is the terminal behavior, not a transitional one.) | Implement handlers for each remaining Go-server-owned endpoint. Remaining: (1) `GET /api/drives/{driveId}` (existing DriveRepo.GetByID), (2) `GET /api/drives/{driveId}/route` (existing routePoints column + decryption). The shared-enum slice (REST-only `not_found` + `invalid_request` on `ErrorPayload.code`, plus `reauth_required` on `subCode`) was completed by MYR-98 on 2026-05-15 so the SDK's `CoreError` is a single union ahead of the endpoint work. | FR-3.3, FR-3.4, NFR-3.5, §6, §7 | `MYR-XX Mount SDK-surface REST endpoints on Go server (drive detail, drive route)` |
