...
- **EQUAL is ALLOWED.** The bound is strictly "later than `now + 30 days`", so a `scheduledFor` exactly 30 days out passes — the same reading the service-window bound takes, and the one that keeps a client rendering a 30-day picker from being refused by its own arithmetic. Clients SHOULD offer at most 30 days and MUST still handle the `400`, since the two clocks are not the same clock.
...
- **Vehicle `in_service` or `offline`** ([MYR-372](https://linear.app/myrobotaxi/issue/MYR-372)) → **hold**. This is the check MYR-313 promised and nothing performed: a car that entered service, or went offline, in the days between the accept and the due instant was nav-dialled anyway, sending a rider out to meet a car that is on a lift. The worker asks **the identical question the instant-accept gate asks** — the `in_service`/`offline` predicate behind `409 vehicle_unavailable` is a single function in the server, read by both surfaces, so a car the accept path would refuse can never be one the sweeper pushes to. **Hold, not expire, and that is the whole semantics:** the reservation stays `accepted`, nothing is claimed and nothing is recorded, and the next tick re-decides — so a car back from service *inside* the lateness window still takes the ride a few minutes late. A car that never returns crosses the 30-minute ceiling and resolves honestly as `failed` / `reservation_expired`. **No new wire status was needed** for any of it: `accepted`-and-retrying and `failed`/`reservation_expired` are both already in this section, so a client can render this today with nothing new to learn.
- **Vehicle status unreadable** (database error) → **hold**, matching every other probe. We cannot tell whether pushing would dial a car on a lift, and a held reservation is recoverable where a wrong push is not. A vehicle row that cannot be found at all is an error too — never a fabricated "dispatchable".
...
- **Pause state unreadable** (database error) → **hold**, matching the busy probe. Every probe on this path fails CLOSED, and since MYR-372 so does the accept path's vehicle read — the two surfaces no longer disagree about what "unknown" means. The difference is only in the shape of the refusal: a human waiting on an accept gets a retryable `500`, while here nobody is waiting and the row is simply re-decided on the next tick.
