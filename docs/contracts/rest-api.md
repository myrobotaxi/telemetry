...
- the car parking at the dropoff (`enroute → completed`) is driven by the **drive-end detector**, not a client — see the transition matrix below.
...
| 2026-07-25 | **MYR-265: rider `POST /api/ride-requests/{id}/board` (accepted→enroute) + autonomous two-leg dispatch.** Added the board endpoint (§7.8): rider-only, idempotent (`enroute` re-board is a 200 no-op), guarded `accepted → enroute`, publishing the internal `ride.boarded` seam that fires the leg-2 **dropoff** nav push. Added `enroute → completed` via drive-end detection. New leg-2 outcome columns `dropoff_dispatch_status`/`dropoff_dispatched_at`/`dropoff_dispatch_error` (migration 0007, P0, off-wire; data-classification.md §1.9). Updated the §7.8 endpoint table, transition matrix, and dispatch-outcome prose. No schema/wire-shape change (existing `RideRequestStatus` enum reused). | go-engineer |
