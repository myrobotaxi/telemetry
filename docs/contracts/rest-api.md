...
| Date | Change | Author |
|------|--------|--------|
| 2026-08-17 | **The caller can edit their own display name: `PATCH /api/users/me` ([MYR-581](https://linear.app/myrobotaxi/issue/MYR-581)). Contracts prose-only — one new endpoint, no new schema type.** Body `{"name": string}`; `200` echoes `{"name"}` as COMMITTED (trimmed), so the client adopts the server's normalization. Refusals: `400` for empty-after-trim (an empty name is a deletion this endpoint does not offer), > 60 chars, control characters, or invalid JSON; `401` unauthenticated; `404` when the authenticated account has no `User` row. **This REVERSES MYR-366's deliberate no-rename rule, by explicit client decision (2026-08-17)** — Apple supplies a name only on the first authorization, so accounts routinely live nameless forever and render as the neutral fallback ("Tesla wants a ride", MYR-532 item 4); the client's directive additionally makes the app-side prompt MANDATORY after update. **Validation is permissive about the world's names** — no ASCII filter and no alphabet allow-list ("José", "Ольга", "李" are names); contrast `InviteLink.inviterName`, which guards a URL parameter. Storage is the next §1.4 single-column carve-out (`store.ProfileNameRepo.UpdateUserName` — self-scoped UPDATE of `User.name`, no migration, `updatedAt` bumped). The name is user content: handlers log the EVENT, never the value. | Claude (go-engineer) |
