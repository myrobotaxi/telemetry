...
| Case | Banner | Why |
|------|--------|-----|
| `declined`, `cancelled`, `reservation_expired` | **always sent** | Outside the design's six phases. Their Activity update carries no alert and expands nothing, so the banner is the only announcement there is — and these are the worst news of a ride. |
