...
**A suspended grant is excluded from the viewer-merge access set.** Not hidden in the catalog, not refused at one gate — *excluded from the set*, which is the single thing the §7.0 vehicle list, the §7.1 snapshot, the WebSocket handshake, the §7.2–§7.4 drives surfaces and the §7.8 rides surfaces all resolve through. One predicate (`suspended_at IS NULL`, in `auth.queryUserVehicleIDs` and the three sharing access statements) kills all of them together, and **no capability flag can out-vote it**: `allowRides: true` on a suspended grant allows nothing, because by the time any capability is read the grant has already failed to appear.
...
The same derivation produces `VehicleSummary.sharePermission` (§5.2.1, §7.0). A consumer that understands `allowRides` MUST prefer it. The old instruction to compare with a cumulative `>=` is **obsolete**: the model is a set of independent flags, not a total order — though a `>=` over the two values actually emitted happens to give the same answer, which is what keeps an un-updated client correct.
...
| `accepted` | — | `decline` (owner, MYR-360 — **SCHEDULED rides only**; instant stays `409`) | `picked-up` (owner, MYR-270) | `409` | `409` | `cancel` (rider, MYR-174) |
