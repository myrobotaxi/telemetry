...
| `vehicle_unavailable` | 409 | **REST-only** | Implemented (MYR-277) | Surface to UI; **do not auto-retry the accept** while the vehicle stays in service / offline — retry once the owner brings it back online / out of service. | An owner tried to **accept** a ride request whose target vehicle cannot currently fulfill it: the vehicle's persisted status is `in_service` (owner put it into service) or `offline` (unreachable). The ride is still legally acceptable (`requested`), so this is **not** `conflict` (an illegal lifecycle *transition*) — it is a capability gate on the vehicle, read from the **current** persisted status at accept time. `parked`/`driving`/`charging` are dispatchable and accept normally. **Accept-only** — decline is never gated. Member of the shared `ErrorPayload.code` enum for single-union SDK typing, but never emitted over the WS transport. See §7.8. |
