...
| `POST` | `/api/ride-requests/{id}/board` | Rider boards an accepted ride (accepted→enroute; triggers the leg-2 dropoff nav push) | Bearer (rider) | FR-9.3 |
...
The `POST`/`GET /api/ride-requests[...]` rows (§7.8) are the P10 ride-hailing surface: the four rider-facing endpoints are mounted as of MYR-174, the three owner-facing endpoints (incoming feed + accept/decline) as of MYR-175, and the rider **board** endpoint as of MYR-265. The autonomous ride flow — `accepted → enroute` (board, MYR-265) and `enroute → completed` (drive-end detection, MYR-265) — is live; the pickup nav push landed with MYR-176 and the dropoff nav push with MYR-265. The `arrived` sub-state and the reschedule endpoints remain with MYR-177 / MYR-192.
...
#### `POST /api/ride-requests/{id}/board`

**Rider-only** (the owner is a party but cannot board → `403 permission_denied`; a non-party → `404`). This is the **leg-2 trigger** of the autonomous two-leg ride (MYR-265): tapping "I'm in" flips the ride `accepted → enroute` (rider aboard, car en route to the **dropoff**). There is **no driver-confirm** — the ride is autonomous.

**Leg semantics (NO new status values — the existing enum is reused):**
- `accepted` = **leg 1**, car en route to the **pickup**. Set on owner accept; the owner-accept handler already pushed the pickup nav (MYR-176 dispatch seam).
- **board** flips `accepted → enroute` = **leg 2**, and publishes an internal `ride.boarded` event carrying the **dropoff** place — the input the nav-dispatch pipeline subscribes to for the Tesla `navigation_request` share push (the car's new destination). Like `ride.accepted` this event is **internal-only**: it never reaches the WS broadcast path, and no Tesla call happens synchronously on this endpoint (the push runs asynchronously off the bus).
- the car parking at the dropoff (`enroute → completed`) is driven by the **drive-end detector**, not a client — see the transition matrix below. **Leg correlation:** the board stamps `enroute_at`, and only a drive-end whose drive **started at/after** `enroute_at` (the leg-2 dropoff drive) completes the ride. A **delayed leg-1 (pickup) drive-end** — whose drive started before board — is ignored, so the ride can never false-complete *at the pickup* while `enroute`. If the leg-2 push failed and the car never departs, there is no leg-2 drive and the ride stays `enroute` (honest v1 behavior).

Legal only from `accepted → enroute`. Responds `200 OK` with the updated `RideRequest` (now `enroute`) and unicasts `ride_status_changed` to both parties.

**Idempotency.** A board when the ride is **already `enroute`** is a `200 OK` no-op that re-returns the current `RideRequest` and does **NOT** re-publish the dropoff dispatch seam (so the dropoff nav is never pushed twice by a client retry). Every other current status is `409 conflict`. Under a concurrent double-tap the guarded `UpdateStatusFrom` (`WHERE status = 'accepted'`) lets exactly one write win — only the winner publishes the `ride.boarded` seam, so **the dropoff nav push is exactly-once per board** (the accept→`ride.accepted` discipline, applied to leg 2).

Errors: `401 auth_failed`, `403 permission_denied` (owner/non-rider party), `404 not_found` (unknown id / non-party), `409 conflict` (not boardable from the current status), `500 internal_error`.

##### Leg-2 dispatch outcome (MYR-265)

The dropoff push outcome is recorded on its **own** triple of columns — `dropoff_dispatch_status` / `dropoff_dispatched_at` / `dropoff_dispatch_error` (migration 0007) — a mirror of the leg-1 (`dispatch_*`) columns so the two legs' histories never clobber each other (a failed dropoff push must not erase the record that the pickup push succeeded). Exactly-once is enforced by BOTH the board transition guard (only the winning `accepted → enroute` write publishes the seam) and, belt-and-suspenders, the `dropoff_dispatched_at` claim latch (a re-delivered `ride.boarded` finds it set and skips). The value sets, retry policy, and internal reason codes are identical to the leg-1 "Dispatch outcome (MYR-176)" contract above. **These leg-2 columns are P0 and NOT exposed on the wire** (data-classification.md §1.9); there is no new WS frame for the dropoff dispatch — the client-visible board signal is the `enroute` `ride_status_changed`. **Interrupted-dispatch reconciliation is leg-1-only in v1:** a crash in the dropoff claim→record window leaves `dropoff_dispatched_at` set / `dropoff_dispatch_status` NULL; impact is low (the car keeps its valid pickup nav and the ride still completes on drive-end), and a follow-up can extend the startup reconciler if leg-2 interruptions prove material.
...
| `accepted` | — | `409` | `board` (rider, MYR-265) | `409` | `409` | `cancel` (rider, MYR-174) |
| `enroute` | `409` | `409` | — | live-tracking (MYR-177) | drive-end (MYR-265) | `409` |
...
- **MYR-265** wires the two remaining autonomous transitions: `accepted → enroute` via the rider **board** endpoint (which also fires the leg-2 dropoff nav push), and `enroute → completed` via the **drive-end detector** — when the autonomous car parks at the dropoff, the drive that ends is joined to the vehicle's in-flight `enroute` ride and completes it (a drive-end for a car with no active ride is a no-op). Completion is **leg-correlated**: the board stamps `enroute_at` and only a drive that started at/after it (the leg-2 dropoff drive) may complete the ride, so a delayed leg-1 pickup drive-end cannot false-complete it. Both transitions emit the usual `ride_status_changed` summary. The `arrived` sub-state stays **MYR-177** territory and is skipped in v1 (the car goes `enroute → completed` directly).
