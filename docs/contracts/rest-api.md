...
- **ActivityKit pushes are untouched** (§7.21.4). An Activity update already addresses exactly one card through its own update token, so it has nothing to collapse against, and the ETA ticker's conserving-priority updates are already coalesced on Apple's own terms.
