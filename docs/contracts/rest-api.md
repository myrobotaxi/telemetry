...
| `vehicle_unavailable` | 409 | **REST-only** | Implemented (MYR-277, MYR-266) | Surface to UI; **do not auto-retry the accept** while the vehicle stays in service / offline / on another ride — retry once the owner brings it back online / out of service, or its current ride ends. | An owner tried to **accept** a ride request whose target vehicle cannot currently fulfill it, for either of two capability reasons: **(a)** the vehicle's persisted status is `in_service` (owner put it into service) or `offline` (unreachable) — MYR-277; or **(b)** the vehicle is already **committed to another active ride** (`accepted`/`arrived`/`enroute`) — the per-vehicle one-active-ride guard (MYR-266): a car serves at most one ride at a time. **Both cases apply to INSTANT rides only** — a ride with `scheduledFor` set skips the status gate (MYR-313) and is outside the per-vehicle index's `scheduled_for IS NULL` predicate (MYR-266), so a reservation is never refused `vehicle_unavailable`. In both cases the ride being accepted is still legally acceptable (`requested`), so this is **not** `conflict` (an illegal lifecycle *transition*) — it is a capability gate on the vehicle. Case (a) reads the current persisted status; case (b) is arbitrated by the partial unique index `uq_go_ride_requests_active_instant_vehicle` (23505) on the guarded `requested`→`accepted` write, so two concurrent accepts for one car resolve to exactly one winner and one `vehicle_unavailable`. `parked`/`driving`/`charging` (and not already booked) are dispatchable and accept normally. **Accept-only** — decline is never gated. Member of the shared `ErrorPayload.code` enum for single-union SDK typing, but never emitted over the WS transport. See §7.8. |
...
- `vehicle_unavailable` (HTTP 409) is emitted only over REST when an owner **accepts** a ride whose target vehicle cannot currently serve it — either because the vehicle is `in_service`/`offline` (MYR-277) or because it is already committed to another active ride (`accepted`/`arrived`/`enroute`, the per-vehicle one-active-ride guard MYR-266) (§7.8). Like `conflict` it is request-oriented and has no WS analogue — the WS path never carries an accept. It is a member of the shared `ErrorPayload.code` enum (added by MYR-277) so the SDK's `CoreError` union stays one enum across transports; the schema enum description marks it REST-only-on-the-wire. Distinct from `conflict` (an illegal lifecycle *transition* on a known ride): the ride IS in an acceptable `requested` state — the *vehicle* just cannot currently serve it. **Accept-only**: decline is never gated on vehicle status. The per-vehicle guard is the exact analogue of the per-rider `ride_active` guard (one active instant ride per rider, MYR-230) applied to the vehicle; scheduled rides are exempt from both.
...
> - `reservation_expired_vehicle_busy` (MYR-179) — a **scheduled** ride came due but its vehicle was still committed to another active ride for the whole busy-hold window (`scheduledFor + 30 min`), so the reservation was resolved `failed` **without** a nav push. Reservation-only: an instant ride can never record it. See "Reservation-time dispatch" below.
>
...
**The sweeper.** A background ticker (default **30 s**, `internal/dispatch` `ReservationSweeper`, wired in `cmd/telemetry-server/reservation_wiring.go`) selects reservations that are `scheduled_for IS NOT NULL AND status = 'accepted' AND dispatched_at IS NULL AND scheduled_for <= now`, oldest reservation first. For each it claims the **same** `dispatched_at` latch the instant path uses and then runs the **same** push pipeline (VIN + token resolution, `navigation_request`, bounded retry, outcome record). Reusing both is what gives scheduled dispatch the identical guarantees rather than a parallel set:
...
- **Kill-switches.** `RESERVATION_DISPATCH_ENABLED=false` stops the sweeper entirely — due reservations stay accepted, unclaimed and outcome-absent, so re-enabling picks up the ones still inside their hold window instead of having burned them. `DISPATCH_ENABLED=false` still applies to reservations too, recording `skipped` with no Tesla call exactly as for an instant accept.
...
**Vehicle-busy hold policy (30 min).** Availability is re-checked at the reservation instant — the precondition MYR-313 deferred here when it exempted scheduled accepts from the `vehicle_unavailable` gate. Before claiming, the sweeper asks whether the vehicle is currently committed to an active **instant** ride (`scheduled_for IS NULL AND status IN ('accepted','arrived','enroute')` — character-for-character the `uq_go_ride_requests_active_instant_vehicle` predicate, shared in code with the `hasActiveRide` catalog flag so the two can never disagree):

- **Busy, within `scheduledFor + 30 min`** → **hold**: no claim, no outcome, no push. The row is left exactly as it was and retried on the next tick, so the moment the car finishes its ride the reservation dispatches normally. Re-pointing the navigation of a car that is carrying another rider is the outcome this exists to prevent.
- **Busy, past `scheduledFor + 30 min`** → **claim and fail honestly**: `dispatchStatus: "failed"` with `dispatch_error = reservation_expired_vehicle_busy`, **without** pushing nav. Claiming here is the point — it converts a silently-pending row into a resolved, alertable outcome on the existing `dispatchStatus` surface and stops the sweeper re-evaluating it forever.
...
The deadline is anchored on `scheduledFor` itself, not on when the sweeper first saw the row, so a restart mid-hold resumes the same deadline rather than resetting it.

**`ride.due` seam.** When a reservation comes due and the sweeper **wins its claim**, it publishes an internal `ride.due` event (ids + `scheduledFor` + the actual `dueAt` instant). It is **internal-only** — never broadcast to WS clients, no REST surface — and exists so the planned rider push notification ("your car is on the way") has a seam without re-opening the dispatch path. **No consumer exists yet.** The publish is fire-and-forget and drop-safe: a bus failure is logged and the nav push proceeds regardless. It is exactly-once per due ride (only the claim winner publishes) and is **not** published on the busy-expiry path — the car never came, so a "your car is on the way" signal would be false.

**v1 boundaries.** Because the busy predicate excludes scheduled rides (mirroring the index), one reservation coming due does not mark the car busy for another: two reservations for the same vehicle at overlapping times will both dispatch. Reservation/reservation collision is out of scope for v1 and belongs with booking-time conflict detection, not dispatch.

#### `POST /api/ride-requests/{id}/decline`

Owner-only. Legal only from `requested` → `declined`; any other current status is `409 conflict`. Responds `200 OK` with the updated `RideRequest` and unicasts `ride_status_changed`. Same error catalog as accept **minus the dispatch seam and the `vehicle_unavailable` gate** — an owner may always decline a request regardless of the vehicle's status.
...
| `accepted` | — | `409` | `picked-up` (owner, MYR-270) | `409` | `409` | `cancel` (rider, MYR-174) |
...
| Date | Change | Author |
| 2026-07-27 | **Reservation-time dispatch — a scheduled ride's pickup nav now fires at `scheduledFor`, not at accept ([MYR-179](https://linear.app/myrobotaxi/issue/MYR-179)).** v1 changes only **WHEN** the existing leg-1 (pickup) push runs. **No new lifecycle status, no new wire field, no wire-shape change, and NO MIGRATION** — the MYR-176 latch columns (`dispatch_status`/`dispatched_at`/`dispatch_error`) are reused unchanged. **Accept half:** the nav-dispatch pipeline no longer pushes when the accepted ride carries `scheduledFor`; it returns without touching the latch, leaving the row latch-unclaimed and outcome-absent (deliberately NOT `skipped` — that is the kill-switch outcome and would both misreport the ride and latch it out of the sweep). Observable on the wire as `dispatchStatus`/`dispatchedAt` staying **absent** after a scheduled accept until the reservation instant. **Instant accepts are unchanged.** **Sweeper half:** a new `internal/dispatch` `ReservationSweeper` ticks every 30 s over `scheduled_for IS NOT NULL AND status = 'accepted' AND dispatched_at IS NULL AND scheduled_for <= now` (oldest first, capped per sweep; the sweeper's own clock is passed into the query rather than using `NOW()` so ONE clock governs both due-selection and the busy-hold deadline), claims the **same** `dispatched_at` latch and runs the **same** push pipeline via a `runLeg` → claim + `runClaimedLeg` split. Reuse is the design: **exactly-once across replicas** (the latch admits one winner, so every server may sweep with no coordination) and **crash safety** come for free — a crash between claim and outcome leaves the identical `dispatched_at` set / `dispatch_status` NULL orphan the **existing leg-1 startup reconciler** already resolves; its query filters on the latch columns only and has never mentioned `scheduled_for`, so scheduled rows were already in scope and **it required no widening** (now pinned by a store test). **Vehicle-busy hold (30 min)** — the reservation-time availability re-check MYR-313 deferred here: before claiming, the sweeper tests the per-vehicle busy predicate (`scheduled_for IS NULL AND status IN ('accepted','arrived','enroute')`, character-for-character `uq_go_ride_requests_active_instant_vehicle` and now SHARED in code with the `hasActiveRide` catalog flag so a third reader cannot drift). Busy within `scheduledFor + 30 min` → **hold** (no claim, no outcome, no push; retried next tick, so the ride dispatches the moment the car frees up); busy past it → **claim and fail honestly** `failed` / new internal code **`reservation_expired_vehicle_busy`**, without a push, so a silently-pending row becomes an alertable `dispatchStatus`; busy state unreadable → hold, never claim. The deadline is anchored on `scheduledFor`, so a restart mid-hold resumes rather than resets it. **New internal `ride.due` seam** (`events.RideDueEvent`, ids + `scheduledFor` + `dueAt`): published once per due ride by the claim WINNER only, internal-only (never broadcast, no REST surface), fire-and-forget and drop-safe, **no consumer yet** — a hook for the planned rider push notification; NOT published on the busy-expiry path (the car never came). **New kill-switch `RESERVATION_DISPATCH_ENABLED`** (default true, `strconv.ParseBool` fail-fast like `DISPATCH_ENABLED`): false stops the sweeper entirely, leaving reservations unclaimed so re-enabling picks them up rather than having burned them; `DISPATCH_ENABLED=false` still records due reservations `skipped` exactly like instant accepts. **v1 boundary:** the busy predicate excludes scheduled rides (mirroring the index), so two reservations for one car at overlapping times both dispatch — reservation/reservation collision belongs to booking-time conflict detection, not dispatch. §7.8 accept section, new "Reservation-time dispatch" subsection, `dispatch_error` code list, and transition-matrix note updated. Implementation: `internal/dispatch/{reservation_sweeper,reservation_dispatch,dispatcher_leg}.go`, `internal/store/ride_request_reservation{,_queries}.go`, `internal/config/load_dispatch.go`, wired in `cmd/telemetry-server/reservation_wiring.go`. | go-engineer |
