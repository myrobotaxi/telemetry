...
The field is **OMITTED** (never an empty string) in exactly one case: the rider has **no `"User"` row at all**. It is surfaced on the party-only detail (`GET /api/ride-requests/{id}`) **and every list item** (rider list, owner incoming feed) — the list path resolves all requesters in a single batched `"User"` lookup (no N+1). The same value rides the reactive WS `ride_request_created` / `ride_status_changed` summary frames (websocket-protocol.md §4.7–4.8). **Classification:** P1 PII, party-only surface, never logged (data-classification.md §1.9).
...
| Date | Change | Author |
| 2026-07-11 | **Populate `requesterName` on ride-request surfaces — P10 ride-hailing ([MYR-229](https://linear.app/myrobotaxi/issue/MYR-229)).** The optional `requesterName` field (contracts v0.11.0, `$defs.RideRequest` + the `ride_request_created` / `ride_status_changed` WS payloads) is now populated server-side from the requester's (`rider_id`) identity — resolved from the Prisma-owned `"User"` table (READ-ONLY, CG-DL-9) via the fallback chain **first name (first whitespace token of the display name) → email local-part → `"Rider"`**, OMITTED only when the rider has no `"User"` row (never an empty string). Surfaced on the party-only REST detail + every list item and carried on the two per-party-unicast WS summary frames. Lists resolve every requester in a **single batched `WHERE "id" = ANY(...)` lookup** (no N+1); single-row reads/mutation-returns use a single lookup, and `Create` resolves BEFORE the INSERT so a name-lookup failure never orphans a committed row. New §7.8 "Requester name (MYR-229)" subsection; §4.7/§4.8 field tables + examples updated; data-classification.md §1.9 gains a "Derived field — `requesterName`" note (**P1 PII, party-only, never logged**; no new stored column). Implementation: `internal/store/ride_request_requester.go` (fallback + single/batched resolvers), threaded through `ride_request_{repo,repo_list,repo_reschedule,types,scan}` → `cmd/telemetry-server/ride_request_adapters.go` → `internal/telemetry/ride_request_{types,wire,handler}.go` → `internal/events/ride_events.go` → `internal/ws/{messages,ride_broadcast}.go`. | go-engineer |
