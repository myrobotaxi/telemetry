...
| Motion / availability | `status`, `gearPosition`, `rideShareEnabled`, `serviceEstimatedEndAt` | Whether the car is available and moving. `gearPosition` is `status`'s `x-atomic-group: gear` sibling and travels with it; it is motion state, not an actuable control. |
