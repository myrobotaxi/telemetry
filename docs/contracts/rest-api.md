...
| **DV-20** | **New (reduced scope)** | SDK-surface REST endpoints not yet mounted on the Go server | The four Go-server-owned endpoints in §7 (snapshot, drives list, drive detail, drive route) are not mounted. `cmd/telemetry-server/main.go` registers only `/api/ws`, `/api/vehicle-status/{vin}`, and `/api/fleet-config/{vin}`; the four SDK paths return `404 not found` from the placeholder catch-all handler today. (The §7.5 invite endpoints and §7.6 `DELETE /api/users/me` are NOT in DV-20's scope -- per DV-23 (RESOLVED 2026-05-08, MYR-69) they are served by the Next.js app and are tracked under MYR-70 / MYR-71 / MYR-73 instead. The Go server returning 404 on those paths is the terminal behavior, not a transitional one.) | Implement handlers for each Go-server-owned endpoint in §6 / §7. Order of implementation: (1) `GET /api/vehicles/{vehicleId}/snapshot` (simplest -- existing VehicleRepo + decryption), (2) `GET /api/drives/{driveId}` (existing DriveRepo.GetByID), (3) `GET /api/drives/{driveId}/route` (existing routePoints column + decryption), (4) `GET /api/vehicles/{vehicleId}/drives` (requires new DriveRepo.List method with cursor-based pagination). Also add the REST-only error codes `not_found` and `invalid_request` to the shared `ErrorPayload.code` enum in `schemas/ws-messages.schema.json` even though the WS never emits them, so the SDK's `CoreError` is a single union. | FR-3.2, FR-3.3, FR-3.4, NFR-3.5, §6, §7 | `MYR-XX Mount SDK-surface REST endpoints on Go server (snapshot, drives list, drive detail, drive route)` |
