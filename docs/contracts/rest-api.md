...
**The anchor is per ACTIVITY and it is in the database** (`go_live_activities`, migration 0027 — `progress_leg`, `progress_source`, `progress_baseline`, `progress_value`; [`data-classification.md`](data-classification.md) §1.18). Two reasons, either sufficient. The ETA ticker is **not sharded across replicas** — every process lists every live Activity and pushes to all of them — so two in-memory baselines for one ride would make the arrow oscillate between two answers every 75 seconds. And a deploy mid-ride would forget the baseline entirely. It is written **after** a delivery Apple accepted, never before, because the clamp's floor must record what the phone HAS SEEN; advancing it on a push that never arrived is exactly how the delivered sequence would come to contain a decrease. A **token rotation preserves it** (§7.21.1's upsert names its SET list), because a rotation is the same Activity on the same leg.
...
| No active nav route (not dispatched, nav cleared) | **no key** | headline + meet-at, no track |
| Car reports minutes but no distance | the fraction, from the ETA ratio | a track that advances |
| Telemetry gone quiet mid-leg (> 3 min) | the **last delivered** fraction, held | a frozen track — and the card's own stale-date says why |
