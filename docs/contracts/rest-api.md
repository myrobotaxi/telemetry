...
| [`ride_request_owner_handler.go`](../../internal/telemetry/ride_request_owner_handler.go) — `upcomingForVehicle` **and** `activeForVehicle` both present | 400 | `ErrCodeInvalidRequest` | The two vehicle filters are mutually exclusive (MYR-400, §7.8). They select disjoint slices, so no single page could honour both; refusing beats resolving by precedence and handing a client a confidently wrong page |
...
**`active` and `upcoming` are COMPLEMENTARY, never overlapping.** A future, dormant reservation is in `upcoming` and **not** in `active`; the same reservation moves into `active` once it goes live. That boundary is the point of having two params rather than one fuzzy "for this vehicle" filter. **Asking for both at once is `400 invalid_request`** — they select disjoint slices, so no single page could honour both, and resolving by precedence would hand a client a confidently wrong page, which is the exact failure mode this read exists to eliminate.
