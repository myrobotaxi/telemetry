...
- **Self-serve provisioning (MYR-257).** On a successful code→token exchange the
  ...
  idempotent, transactional path — NOT the identity module). In one transaction
  it upserts `"User"` (`ON CONFLICT ("id") DO NOTHING`, id = the caller's JWT
  `sub` / go_users id — no mapping table), `"Settings"` (`teslaLinked=true`), and
  `"Account"` (`ON CONFLICT ("provider","providerAccountId") DO UPDATE`,
  dual-write-encrypted tokens; `providerAccountId` is the Tesla OIDC `sub` from a
  `userinfo` call). This makes a brand-new go_users-native Apple user a working
  owner with **no ops step** — the `AUTH_APPLE_BOOTSTRAP` + pre-seeded-`Account`
  MVP steps are gone. A denied/failed link provisions nothing (no orphan
  `"User"`). Design + ownership rationale + the required cross-repo schema gate:
...
- **Server-side stream setup (best-effort).** After provisioning, the callback
  ...
  `GET /api/1/vehicles`), seeds their `"Vehicle"` identity rows, and — only when
  the tesla-http-proxy is configured at runtime — pushes the fleet-telemetry
  config per VIN so the car streams without an ops `fleet-config push`. Any
  failure here is logged and never fails the link. The virtual-key approval stays
  a Tesla-app user action.
...
| Date | Change | Author |
| 2026-07-24 | **Fully self-serve owner onboarding — provision on Tesla link ([MYR-257](https://linear.app/myrobotaxi/issue/MYR-257)).** §7.11.2 `GET /api/tesla/link/callback` behavior change: on a successful code→token exchange the callback now PROVISIONS the caller's minimal Prisma owner rows instead of requiring a pre-seeded `Account`. New `store.OwnerProvisioner` (a single, audited, idempotent, transactional writer — NOT the identity module, keeping ADR-001 §4's identity/`User`-read-only boundary intact) upserts `"User"` (`ON CONFLICT ("id") DO NOTHING`, id reused from the caller's go_users id / JWT `sub` — no mapping table), `"Settings"` (`teslaLinked=true`), and `"Account"` (`ON CONFLICT (provider, providerAccountId) DO UPDATE`, dual-write-encrypted tokens; `providerAccountId` = Tesla OIDC `sub` from a new `teslaauth.FetchUserInfo` call) in one transaction. A brand-new go_users-native Apple user becomes a working owner with **zero ops steps** (`AUTH_APPLE_BOOTSTRAP` + pre-seeded-`Account` MVP steps deleted); a denied/failed link provisions nothing (no orphan `"User"`). A best-effort post-link hook lists the owner's vehicles (Fleet `GET /api/1/vehicles`, new `FleetAPIClient.ListVehicles`), seeds `"Vehicle"` identity rows, and — only when the tesla-http-proxy is configured at runtime — pushes the fleet-telemetry config per VIN so the car streams without an ops `fleet-config push` (guarded so no live push ever runs in tests). `account_not_provisioned` is now legacy (unreachable on the happy path); `persist_failed` widens to cover userinfo/transaction failure. Data-lifecycle §1.4 updated: `User`/`Settings`/`Vehicle` gain narrow Go INSERT/upsert access, `Account` widens to upsert. Cross-repo Prisma-schema verification is a required `sdk-architect` gate — see [`../architecture/self-serve-onboarding.md`](../architecture/self-serve-onboarding.md). No new endpoint or request/response shape; no new persisted column. | go-engineer |
