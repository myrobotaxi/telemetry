...
**Party-scoped trip-shape edit ([MYR-541](https://linear.app/myrobotaxi/issue/MYR-541), CLIENT-DIRECTED: "allow the users to change the pick up or drop off at anytime. Obv if pick up has past they can only change drop-off").** Body is contracts `$defs.RideRequestTripPatch` (v0.33.0): `pickup?` and/or `dropoff?` (at least one, else `400 invalid_request`) plus the REQUIRED `tripVersion` the editor holds. Answers `200` with the updated `RideRequest` (whose `tripVersion` incremented by exactly one).
...
- **CONCURRENCY is `tripVersion`**: one UPDATE carries the status window, the version match and the place writes; a stale editor gets `409 conflict` and refetches. Two people editing at once resolve to exactly one winner — never a silent clobber.
- **What follows a winning edit**: the OTHER participants are pushed ("«FirstName» changed the drop-off" / "Your pickup was changed" — the edited PART is named, the PLACE never travels on a lock-screen surface); every live surface converges off the TripEdit-marked `ride_status_changed` frame, whose `tripVersion` is the refetch signal (the frame stays summary-only — no places); and when the edited endpoint is the CURRENT leg's target (pickup edited while `accepted`+dispatched, drop-off edited while `enroute`) the dispatcher RE-SHARES the car's nav through the sequencer and then VERIFIES it end-to-end per MYR-527 — re-share once more on a miss, owner's "may not have the route" push on two. A drop-off edited before the rider's Start re-shares nothing: leg 2 has not dispatched, and the Start dispatches the NEW drop-off for free. A pickup edited on an undispatched reservation likewise: the sweeper re-reads the pickup every tick, so the MYR-535 dispatch lead re-computes on its own.
