...
|-------|------|-------|-------|
| `acceptedByName` | `string` | **P1** | **OPTIONAL.** The FIRST NAME of the account that **REDEEMED** this invite — the name the shared user themselves entered at signup. Resolved by the platform's THREE-SOURCE IDENTITY LADDER (Prisma `"User".name` → the Apple first-consent name in `go_identity_apple` → `go_users`) and reduced to its first whitespace-separated token: the SAME resolution and the SAME reduction behind `VehicleSummary.ownerFirstName` (§7.0), `RideRequest.requesterName` (§7.8) and `RedeemShareInviteResponse.ownerFirstName` (§7.5.6). **OMITTED** on a `pending` row and on an `accepted` row whose account has no resolvable name — the two are deliberately INDISTINGUISHABLE, because `status` already tells them apart and a second sentinel would be a state with no copy. |
...
##### Nameless-owner gate on create and accept ([MYR-581](https://linear.app/myrobotaxi/issue/MYR-581))

A create is refused **`409 vehicle_unavailable`**, **with NO `subCode`**, and the message `This vehicle is not available for ride requests`, when the target vehicle's **OWNER has no display name** on any identity rung — i.e. when `VehicleSummary.ownerFirstName` (§7.0) resolves to `null` for that car. The same refusal is enforced again on **accept**.
...
- **The vehicle's OWNER has no display name** ([MYR-581](https://linear.app/myrobotaxi/issue/MYR-581)) → **hold**, on the same terms again. This is the THIRD and last enforcement layer of the nameless-owner gate (§7.8 create), and the only one that runs outside a request — but the reason it exists is **monotonicity, not reversibility**, and the distinction changes what it is for. A pause can be flipped back on, so its dispatch layer guards a live condition; namelessness can only ever resolve FORWARD (no path in the server un-names an account), so this layer's population is **reservations accepted before the create gate shipped**. **HOLD rather than expire** is nonetheless the right semantics for the same reason as the pause's: an owner who sets a name inside the lateness window still gets the dispatch they meant to allow, while one who never does lets the row resolve at `scheduledFor + MaxLateness` as `failed` / `reservation_expired` — no new resolution path, no new wire status, nothing claimed and nothing recorded while held. It rides the SAME single joined read as the two arms above (one statement, three facts) and reads a BOOLEAN, so the sweep never handles the P1 name.
...
**P1.** The name is user content: the handler logs the EVENT and the user id, never the value.
