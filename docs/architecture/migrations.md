...
>
> The sanctioned pattern is a binary under `cmd/` backed by a package under `internal/store/`, run deliberately by an operator rather than implicitly at server startup. Five exist today: `cmd/backfill-account-tokens`, `cmd/backfill-vehicle-gps`, `cmd/backfill-route-blobs` (each sealing legacy plaintext into a ciphertext column), `cmd/purge-plaintext-columns` (MYR-433, removing the plaintext once sealed), and `cmd/backfill-name-confirmations` (MYR-583). Follow their shape: idempotent, batched, `-dry-run` where the change is destructive, and a machine-readable report on stdout with a non-zero exit when rows were skipped.
>
> **MYR-583 is the smallest instance of the pattern and shows where it bends.** Its two statements — clearing the legacy `'Tesla User'` placeholder from `"User"."name"` and backfilling the Go-owned `go_profile_name_confirmations` table it could not populate from a migration — affect a handful of rows between them, so it runs as **ONE transaction rather than a batched pass**: the run is then a single reviewable event (either both landed or the database is untouched) and the audit row for the scrub commits with the write it describes rather than merely before it. Batching exists to keep a long pass from holding locks; where there is no long pass, atomicity is the better trade. Everything else is unchanged: `-dry-run` counts and rolls back, a re-run affects zero rows, and the report is JSON on stdout.
>
