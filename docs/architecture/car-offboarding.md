...
**Status:** Design + Fleet API verification (MYR-258), now **implemented** — `DELETE /api/tesla/vehicles/{vehicleId}`
backed by `store.OwnerTeardown` (see [`rest-api.md`](../contracts/rest-api.md) §7.12 and
[`data-lifecycle.md`](../contracts/data-lifecycle.md) §1.4). Built after
[MYR-257](https://linear.app/myrobotaxi/issue/MYR-257) (`store.OwnerProvisioner`) merged, as its exact inverse.
This doc is the exact inverse of
...
- **What "delete our tokens locally" does vs. does not do:** deleting the `Account` row destroys *our* copy of the tokens (so we can no longer call Tesla), but it does **not** revoke the grant — the app stays listed in the owner's connected apps and the refresh token remains technically valid at Tesla until it expires or the owner revokes. **Local token deletion ≠ grant revocation.**
- **UPDATED BY [MYR-366](https://linear.app/myrobotaxi/issue/MYR-366): there IS a partner machine call, and we now make it.** Tesla's OAuth2 revocation endpoint `POST https://auth.tesla.com/oauth2/v3/revoke` (RFC 7009) accepts the stored **refresh** token plus our `client_id` and kills the whole grant with no human in the loop. The server issues it on a **last-vehicle** teardown and on account deletion, **before** the local delete destroys the token the call needs. It is best-effort: a Tesla failure never blocks the teardown. The consent-revoke page below is therefore now the **fallback** for a failed revocation, not the only route — and it is still what removes a virtual key, which no API can.
...
| Clear our Tesla tokens | Backend | **Yes** | Delete/clear the `Account` (tesla) row | Removes *our* access. |
| Revoke the grant at Tesla | Backend | **Yes** (MYR-366) | `POST auth.tesla.com/oauth2/v3/revoke` with the stored refresh token + `client_id`, **before** the row is cleared | Best-effort; last-vehicle removals only. On failure the consent page below is the fallback. |
...
- **OAuth revoke IS a partner machine call — corrected by [MYR-366](https://linear.app/myrobotaxi/issue/MYR-366).** `POST auth.tesla.com/oauth2/v3/revoke` (RFC 7009) revokes the grant with only the stored refresh token and our `client_id`. The server issues it on last-vehicle teardowns and on account deletion, ahead of the local token delete. The owner-confirmed consent page `auth.tesla.com/user/revoke/consent?revoke_client_id=…` is retained as the fallback when that call fails, and remains the only route that also removes the app from the tesla.com third-party-apps list for a grant we could not reach.
