...
| `live activity: gave up on held ends; cards were never ended` | A completed card's `end` could not be delivered for **30 minutes** after the ride finished, so it was abandoned | A real failure with a rider on the other end. Look upward for the repeated `end not delivered` warnings and their APNs error — a `403 TopicDisallowed` means `APNS_TOPIC` is wrong; a run of 5xx means Apple was unreachable for half an hour |
| `live activity ticker: held-end pass yielded on its time budget` | More completed cards were due than one pass may spend half a tick on | Expected after an outage backlog; the deferred rides are retried next pass. Persisting means sends are slow — check APNs latency before raising `MaxPerPass` |
